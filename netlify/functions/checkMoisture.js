...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    for (const uid of Object.keys(users)) {
      ...
      for (const deviceId of Object.keys(userData.devices)) {
        ...
        if (typeof display === "number" && display < THRESHOLD) {
          // Csak akkor küldünk, ha legalább 10 perc eltelt az utolsó riasztás óta
          if (now - lastAlertTs > MIN_INTERVAL_MS) {
            needAlert = true;
            // Ezt a device-ot jelöljük frissen értesítettnek
    ...
    // Először mentsük az időbélyegeket
    if (Object.keys(updates).length > 0) {
      await db.ref().update(updates);
      console.log("⌚ lastAlertTs frissítve az érintett eszközökre.");
    }

    // Majd küldjük az értesítést az összes feliratkozottnak
